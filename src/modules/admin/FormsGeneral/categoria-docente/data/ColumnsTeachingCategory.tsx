...
export const ColumnsTeachingCategory = ({ handleDelete, handleEdit }: Props): IColumn<ITeachingCategory>[] => [
    {
      ...
      render: (row) => (
        <div className="flex items-center justify-center gap-1">
          <button onClick={() => handleEdit(row.nLegCatCodigo)} className="bg-indigo-600 text-secondary-100 p-1.5 rounded-lg hover:bg-indigo-700 transition-colors duration-300">
            <FaRegEdit size={14} />
          </button>
          <button onClick={() => handleDelete(row.nLegCatCodigo)} className="bg-red-600 text-secondary-100 p-1.5 rounded-lg hover:bg-red-700 transition-colors duration-300">
